...
- `players.broadcast{...}` — тот же фильтр, что у `players.list`
  (`alive`, `team`, `bot`, `hltv`, `name`), сужает рассылку до подмножества.
- `players.broadcast` умеет не только слать, но и действовать на всех сразу:
  `health`, `armor`, `maxspeed`, `freeze`, `godmode`, `noclip`, `team`,
  `spawn`, `money`, `give`, `strip`, `ammo`, `clip`, `drop`, `slay`, `slap`.
  Без значения (там, где у игрока это чтение) бросает ошибку — общего ответа
  на всех нет, читай через `players.list()`.
