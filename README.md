...
- **Своё пространство имён у плагина.** Папка со своим `require` и своим
  окружением: глобалки не текут наружу, ошибка при загрузке снимает плагин
  целиком, а не половину. Это разделение имён, а не песочница — см.
  [Права плагинов](#права-плагинов).
...
приходит в плагин как данные.

Обратная сторона: плагин может повесить сервер обычной ошибкой — вечным циклом
или съеденной памятью. Ограничений по времени и по памяти нет, и это касается не
только чужого кода, но и твоего собственного. Подробнее — в
[Установке](docs/install.md#что-изоляция-не-даёт).
