...
Под капотом используется **LuaJIT 2.1**. Состояние игроков, события геймплея и доступ к движку берутся из **ReHLDS** и **ReGameDLL**. Для работы нужны [metamod-r](https://github.com/rehlds/Metamod-R) и [ReGameDLL_CS](https://github.com/s1lentq/ReGameDLL_CS) — подробности в разделе [Установка](/install).

Цикл разработки быстрый: меняешь код → `lua_reload` → сразу видишь результат. Не нужно перекомпилировать и перезапускать карту, как в AMX Mod X.

Каждый плагин живёт в своём изолированном окружении со своим `require`. При этом у него полный доступ к движку — как у модуля AMXX.
