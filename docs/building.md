...
Просто ставишь модуль на сервер? Бинарники уже собраны — бери в
[релизах](https://github.com/KotyaraKryt/cs-lua/releases), эта страница не
нужна. Она — для правок в C++ или сборки текущего `main`.
...
`CSLUA_SERVER_DIR` кладёт модуль, `core/` и `lib/` в `<gamedir>/addons/lua/`
на этапе сборки:
...
Самого тестового сервера в репозитории нет — это игровой контент Valve. Что
нужно поставить рядом — в [Установке](install.md).
...
Игра не нужна: `core/` и `lib/` — обычный Lua, а пространства имён движка
