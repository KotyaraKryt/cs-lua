...
|  |  |
|---|---|
| [`timer.after`](after.md) | Вызывает функцию один раз через заданное время |
| [`timer.every`](every.md) | Вызывает функцию каждые N секунд, пока её не снимут |
| [`timer.cancel`](cancel.md) | Снимает таймер по id |
...
|  |  |
|---|---|
| [`timer.create`](create.md) | Заводит именованный таймер; повторный вызов заменяет старый |
| [`timer.destroy`](destroy.md) | Снимает именованный таймер этого плагина |
