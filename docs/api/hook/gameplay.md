...
Игра вот-вот решит, может ли жертва вообще получить урон от этого атакующего — самая ранняя точка, ещё до того, как известно, сколько урона.
...
**Отмена.** `e:cancel()` принудительно запрещает урон — независимо от того, что решили бы правила игры (тимфрайр и т.д.) сами. Разрешить урон, который правила и так запретили бы, этим событием нельзя: только забрать разрешение, не выдать его.

В отличие от `player:hurt`, `e.attacker` здесь не обязан быть игроком — источник урона ещё не сведён к конкретному числу.
