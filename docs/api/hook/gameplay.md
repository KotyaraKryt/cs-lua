...
## player:round_respawn {#player_round_respawn}

Игрока вот-вот сбросят на новый раунд — позиция, здоровье, оружие (оставить или нет), наказание за тимкилл.

```lua
hook.add("player:round_respawn", id, function(e)
	...
end)
```

### Поля события {#player_round_respawn-поля события}

| поле | тип |  |
|---|---|---|
| `e.player` | player | кого касается событие |

**Отмена.** `e:cancel()` — для этого игрока сброс на новый раунд не происходит вообще, он остаётся как есть.

Не путать с `player:spawn` — тот срабатывает и на самый первый спавн игрока тоже, это событие — именно про сброс между раундами.
...
- [player:spawn](gameplay.md#player_spawn)

## player:give_default_items {#player_give_default_items}

Игроку вот-вот выдадут стандартный набор на раунд — нож, дефолтный пистолет, всё, что задано cvar-ами команды.

```lua
hook.add("player:give_default_items", id, function(e)
	...
end)
```

### Поля события {#player_give_default_items-поля события}

| поле | тип |  |
|---|---|---|
| `e.player` | player | кого касается событие |

**Отмена.** `e:cancel()` — движок сначала снимает с игрока всё, что у него есть, и только потом выдаёт стандартный набор; отмена пропускает оба шага, инвентарь остаётся как был.
