...
|  |  |
|---|---|
| [`mysql.connect`](connect.md) | Открывает соединение с сайтовой базой |

## Объект соединения

|  |  |
|---|---|
| [`conn:query`](query.md) | Выполняет запрос и отдаёт строки в коллбэк |
| [`conn:exec`](exec.md) | То же самое, что conn:query — имя удобнее для запросов без строк |
| [`conn:close`](close.md) | Закрывает соединение |
