...
""",
                ...
                {
                    'name': 'p:motd',
                    'brief': 'Открывает игроку панель MOTD с текстом.',
                    'sig': 'p:motd(text)',
                    'args': [('text', 'string', 'до 1536 байт после перекодировки')],
                    'example': 'p:motd(table.concat(lines, "\\n"))',
                    'extra': """
...
""",
                    'notes': [
                        ('note', 'Заголовок окна задаёт клиент, а не сервер: в протоколе его нет.'),
                        ('warning', 'Длиннее 1536 байт обрезается. Считать надо после перекодировки:\nв CP1251 кириллица занимает вдвое меньше, чем в UTF-8.'),
                    ],
