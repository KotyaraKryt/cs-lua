...
""",
        ...
        },
        {
            ...
            'items': [
                ...
                {
                    'name': 'sv.maps',
                    'brief': 'Список карт, установленных на сервере.',
                    'sig': 'sv.maps()',
                    'args': [],
                    'returns': [('table', 'имена без `.bsp`, порядок не гарантирован')],
                    'extra': 'Читает `maps/` на диске напрямую, а не берёт список\nоткуда-то из движка — плагину меню голосования этого достаточно, чтобы\nотсеять карты, которых на самом деле нет.',
                },
                {
                    'name': 'sv.map_exists',
                    'brief': 'Есть ли карта на диске.',
                    'sig': 'sv.map_exists(name)',
                    'args': [('name', 'string', 'имя без `.bsp`')],
                    'returns': [('boolean', '`true`, если `maps/<name>.bsp` существует')],
                    'extra': 'Дешевле, чем звать `sv.maps()` и искать в нём —\nодна проверка файла вместо чтения всей папки.',
                },
                {
                    'name': 'sv.cmd',
                ...
                {
                    'name': 'sv.exec',
                    'brief': 'Выполняет всю очередь sv.cmd() прямо сейчас.',
                    'sig': 'sv.exec()',
                    'args': [],
                    'extra': 'Обычно движок разбирает очередь на следующем кадре. Зови `sv.exec()`, когда эффект команды нужно прочитать в том же вызове — например, выставил cvar через `sv.cmd()` и тут же читаешь его обратно (как `dp_clientinfo` у Dproto).',
                    'example': """
sv.cmd("dp_clientinfo #%d", id)
sv.exec()
local steamid = sv.cvar("dp_clientinfo"):str()
""",
                },
            ],
        },
