...
попадания, а это враньё.''',
                      ...
                      see=[('player:hurt_post', 'player_hurt_post.md'),
                           ('player:hit', 'player_hit.md'),
                           ('player:can_take_damage', 'player_can_take_damage.md')]),
                event('player:hurt_post', 'Урон уже применён; только для наблюдателей.',
                      ...
                      regamedll=True),
                event('player:hit', 'Один хитбокс получил попадание — раньше и точнее, чем `player:hurt`: до брони, до множителей, до суммирования дроби дробовика в одно число.',
                      [('e.victim', 'player', 'кому прилетело'),
                      ...
                      extra='Дробовик даёт одно `player:hit` на каждую долетевшую дробину и одно `player:hurt`/`weapon:fire` в сумме — здесь урон ещё не сложен.',
                      regamedll=True,
                ...
                event('player:heal', 'Игроку вот-вот дадут здоровье — аптечка, админ-команда; своей регенерации в CS нет.',
                      ...
                      regamedll=True),
                event('player:can_take_damage', 'Низкоуровневый гейт «может ли жертва вообще получить урон от этого атакующего» — то, на чём стоит тимфрайр.',
                      [('e.victim', 'player', 'кого могут ранить'),
                       ('e.attacker', 'player \\| nil', '`nil`, если источник урона — не игрок (мир, ловушка, обломки)')],
                      slug='player_can_take_damage',
                      cancel='`e:cancel()` принудительно запрещает — но не всегда действует, см. ниже. Разрешить урон, который правила и так запретили бы, этим событием нельзя: только забрать разрешение, не выдать его.',
                      extra='''По движку (проверено по исходникам ReGameDLL, не по одним только заголовкам) эта проверка реально вызывается **до двух раз на одно попадание**:
...
Значит на удар пулей/ножом от игрока событие прилетит дважды подряд с разным эффектом отмены, на урон не от игрока — один раз (только `TakeDamage`), а на урон от гранаты отмена не сработает совсем.''',
                      regamedll=True,
                      see=[('player:hurt', 'player_hurt.md'), ('player:hit', 'player_hit.md')]),
